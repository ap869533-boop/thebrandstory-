...
interface PlatformContextType {
  ...
  viewParams: { username?: string; citySlug?: string; categorySlug?: string; blogSlug?: string };
  navigateTo: (view: string, params?: { username?: string; citySlug?: string; categorySlug?: string; blogSlug?: string }) => void;
...
export const PlatformProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navigateTo = (view: string, params: { username?: string; citySlug?: string; categorySlug?: string; blogSlug?: string } = {}) => {
    setCurrentView(view);
  ...
  useEffect(() => {
    const fetchLiveDatabaseData = async () => {
      try {
        const [creatorsRes, campaignsRes, enquiriesRes] = await Promise.all([
          fetch('/api/creators'),
          fetch('/api/campaigns'),
          fetch('/api/enquiries'),
        ]);
  ...
  const [siteLogo, setSiteLogo] = useState<string>(() => {
    return localStorage.getItem('sc_site_logo') || '/logo.png';
  });
  ...
  const applyToCampaign = (campaignId: string, creatorId: string, pitch: string) => {
    ...
    // Background sync with Backend REST API
    fetch(`/api/campaigns/${campaignId}/apply`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const updateCreatorProfile = (creatorId: string, updates: Partial<Creator>) => {
    ...
    // Also update authUser if avatar changes so profile photo shows immediately everywhere
    if (updates.avatar !== undefined || updates.coverImage !== undefined) {
      setAuthUser(prev => {
        if (!prev) return prev;
        const updatedUser = { ...prev };
        if (updates.avatar !== undefined) updatedUser.avatar = updates.avatar;
        // Update creatorProfile inside authUser too
        if (prev.creatorProfile) {
  ...
  const addPartnerBrand = async (brand: Omit<BrandPartner, 'id'>) => {
    try {
      const res = await fetch('/api/partner-brands', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
