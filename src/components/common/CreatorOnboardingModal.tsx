...
export const CreatorOnboardingModal: React.FC = () => {
  const { onboardingModalOpen, closeOnboardingModal, registerCreator, navigateTo } = usePlatform();
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    if (step < 3) {
      ...
      const newCreator = registerCreator({
        name: formData.name,
        username: formData.username.replace('@', '').trim() || formData.name.toLowerCase().replace(/\s+/g, ''),
        currentCity: formData.currentCity,
        primaryCategory: formData.primaryCategory,
        phone: formData.phone || '+91 98765 43210',
        email: formData.email || `${formData.username || 'creator'}@gmail.com`,
        followers: Number(formData.followers) || 10000,
        engagementRate: Number(formData.engagementRate) || 4.2,
        avgViews: Number(formData.avgViews) || 8000,
        startingPrice: Number(formData.startingPrice) || 3000,
        bio: formData.bio || `Lifestyle & ${formData.primaryCategory} creator based in ${formData.currentCity}. Open for collaborations.`,
        avatar: formData.avatar,
        pricing: {
          reelPrice: (Number(formData.startingPrice) || 3000) * 1.5,
          storyPrice: (Number(formData.startingPrice) || 3000) * 0.4,
          postPrice: Number(formData.startingPrice) || 3000,
          ugcPrice: (Number(formData.startingPrice) || 3000) * 1.2,
          youtubePrice: (Number(formData.startingPrice) || 3000) * 3,
          eventPrice: (Number(formData.startingPrice) || 3000) * 2,
          isNegotiable: true,
