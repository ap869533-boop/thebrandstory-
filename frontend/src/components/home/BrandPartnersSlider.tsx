      ...
      <div className="w-full overflow-hidden py-0.5 sm:py-1">
        <div className="animate-marquee flex items-center gap-2.5 sm:gap-4">
          {displayBrands.map((brand, idx) => (
            <div
              key={`${brand.id}-${idx}`}
              className="w-28 sm:w-44 md:w-56 h-14 sm:h-20 md:h-24 bg-white rounded-xl sm:rounded-2xl border border-slate-200/90 shadow-xs hover:shadow-md hover:border-blue-300 transition-all duration-300 overflow-hidden flex items-center justify-center shrink-0 group cursor-default p-2 sm:p-3 relative"
            >
              {/* Proper Logo Fit */}
              {brand.logoUrl ? (
                <img
